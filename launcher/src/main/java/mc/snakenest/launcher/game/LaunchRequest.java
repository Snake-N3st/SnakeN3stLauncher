 ...
 * @param memoryMb      heap size, passed as {@code -Xmx<memoryMb>M} - see
 *                      {@code modpack.ModpackSettings}
 * @param extraJvmArgs  raw, space-separated extra JVM args from {@code
 *                      modpack.ModpackSettings}; empty string (not null) when unset
...
public record LaunchRequest(
    ...
    public String toString() {
        return "LaunchRequest[username=%s, offlineUuid=%s, instanceDir=%s, mcVersion=%s, loader=%s, loaderVersion=%s, memoryMb=%d, extraJvmArgs=%s, sn3TokenHex=<redacted>]"
                .formatted(username, offlineUuid, instanceDir, mcVersion, loader, loaderVersion, memoryMb, extraJvmArgs);
    }
}
